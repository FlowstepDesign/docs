...
description: "Input schema, output shape, and examples for all 17 Flowstep MCP tools."
sidebarTitle: "Tools"
---
...
```json
<!-- screenType: "iphone-x-vertical" width: "375" height: "812" name: "Change to a light theme" colorTheme: "blue" answerId: "c13d3707-0efe-49f5-b6cb-0ca5ac5223d0" -->
...
| `label` | `string` | The name of the new screen - defaults to 'Imported from code' if not provided |

**Note** - A comment similar to the one below MUST be present as the first line of the JSX as it is used to add the screen correctly. `screenType`, `name`, and `answerId` are all optional — `answerId` is used (internally) if present (e.g. when passing JSX copied from `get-screen` output) but is not mandatory.
