...
- A **monthly limit** — resets on the **1st of each month**
...
<!-- TODO: verify — per-plan message limits are stored in the database. Confirm the exact numbers for free/starter/pro before publishing. -->
...
Organizations on unlimited plans have `"unlimited": true` in the `limits` object and will see `null` for `max` values. Per-request limits (attachment size, etc.) still apply.
